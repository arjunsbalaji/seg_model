...
main_dir = '/media/arjun/VascLab EVO/projects/oct_ca_seg/new final data/lachy data/49'
...
folders  = [x for x in a if len(x)==9]

i=11001 #make this right
for dawg in sorted(folders)[:2]:

    casedir = os.path.join(main_dir, dawg)
    folders  = [x for x in a if len(x)==3]

    
    sets = [x for x in os.listdir(casedir) if x[0]=='I']

    for gather in sorted(sets):
        #mprint(gather)
        setdir = os.path.join(casedir, gather)
        
        #print(setdir)
        #for case in sorted(os.listdir(setdir)):
        ...
        print('Doing case:', dawg, '-', gather)
        
        for array in sorted(os.listdir(images_dir)):
